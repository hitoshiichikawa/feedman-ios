...
本機能は Feedman iOS の認証入口にパスキーを追加し、Google OAuth のみの状態から App Store Review Guideline 4.8 に適合できる構成へ移行する。サーバ側 #216 / PR #217 の passkey API は WebAuthn ceremony を提供し、認証成功時に既存 native auth と同じ `auth_code` を返すため、iOS 側は既存 `AuthRepository.exchangeAuthCode`、`TokenStore`、`AppEnvironment.completeLogin` を再利用する。

ログイン画面では Issue コメント回答どおり Google を主導線のまま維持し、パスキーでログインとアカウント新規作成を副導線として追加する。新規作成では初回に recovery email を収集せず username のみを送る。サーバ契約上 `POST /api/passkey/registration/finish` は `{user_id}` を返すため、iOS は登録完了後に続けて passkey authentication begin/finish を実行し、取得した `auth_code` を既存 token exchange へ渡す。もしサーバ契約が registration finish で `auth_code` を返す形へ変更された場合は、同じ `PasskeyAuthCodeHandoff` 境界で即時合流に切り替えられる。
...
- 新規コンポーネントの根拠:
  ...
  - `PasskeyAuthCodeHandoff`: registration finish 直後の auth continuation と authentication finish の `auth_code` 合流を 1 箇所に閉じるため。
...
Feedman/
├── Core/
│   ├── APIModels.swift                         # PasskeyAPIModels: passkey request/response DTO を追加
...
├── Features/
│   ├── Login/
│   │   ├── LoginViewModel.swift                # LoginPasskeyFlow: passkey login/signup state machine を追加
...
- `Feedman/Core/APIModels.swift` — `PasskeyRegistrationBeginRequest`、`PasskeyBeginResponse`、`PasskeyAuthenticationFinishResponse`、`PasskeyCredentialEnvelope`、`UserResponse.username?` を追加する。
...
- `Feedman/Features/Login/LoginViewModel.swift` — Google flow を維持しつつ passkey login/signup state を追加する。
...
| 2.6 | signup login handoff | PasskeyAuthCodeHandoff, AuthStateIntegration | `exchangeAuthCode` | Registration to login |
| 2.7 | username error | LoginPasskeyFlow, PasskeyRepository | Error mapping | Signup failure |
| 2.8 | registration cancel/failure | LoginPasskeyFlow | Cancellation mapping | Signup failure |
...
| 4.6 | add failure session preservation | AccountPasskeyEnrollment | Error mapping | Add failure |
...
| 8.2 | View isolation | PasskeyRepository, Coordinator | Protocol boundaries | Implementation |
| 8.3 | secret hygiene | PasskeyRepository, Coordinator | DTO redaction | All ceremonies |
| 8.4 | cancellation mapping | LoginPasskeyFlow, AccountPasskeyEnrollment | Error mapping | All ceremonies |
| 8.5 | mockable tests | PasskeyRepository, Coordinator | Protocol mocks | XCTest |
...
| Requirements | 2.3, 2.5, 3.2, 3.4, 4.2, 4.4, 6.6, 8.3 |

**Responsibilities & Constraints**
- WebAuthn options はサーバ JSON のうち iOS platform request に必要な subset を decode する。
...
|--------|----------|---------|----------|--------|
| POST | `/api/passkey/registration/begin` | `{ username, code_challenge }` | `{ challenge_id, options }` | 400, 409, 429, 500 |
| POST | `/api/passkey/registration/finish` | `{ challenge_id, credential }` | `{ user_id }` | 400, 429, 500 |
| POST | `/api/passkey/authentication/begin` | `{ code_challenge }` | `{ challenge_id, options }` | 400, 429, 500 |
| POST | `/api/passkey/authentication/finish` | `{ challenge_id, credential }` | `{ auth_code }` | 400, 429, 500 |
| POST | `/api/passkey/registration/add/begin` | `{}` + Bearer | `{ challenge_id, options }` | 401, 400, 500 |
...
| Requirements | 2.3, 2.5, 3.2, 3.4, 4.2, 4.4, 4.7, 8.2, 8.5 |
...
protocol PasskeyRepository {
    func beginRegistration(username: String, codeChallenge: String) async throws -> PasskeyBeginResponse
    ...
    func beginAuthentication(codeChallenge: String) async throws -> PasskeyBeginResponse
    ...
    func beginAddingCredential(accessToken: String) async throws -> PasskeyBeginResponse
...
- Invariants: endpoint path と JSON key は #216 contract に一致する。
...
protocol PasskeyPlatformAuthorizationCoordinating {
    ...
    func performAssertion(options: PasskeyPublicKeyCredentialRequestOptions) async throws -> PasskeyCredentialEnvelope
}
```
...
| Requirements | 1.2, 1.5, 2.1, 2.2, 2.3, 2.6, 2.7, 2.8, 3.1, 3.5, 3.6, 3.7, 8.4, 8.5 |
...
- `startPasskeyRegistration(username:)` は username validation → PKCE → registration begin → platform registration → registration finish → passkey login continuation の順に実行する。
...
- Invariants: 失敗・キャンセル時は in-flight PKCE verifier と challenge_id を破棄する。
...
| Requirements | 4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.7, 6.6, 7.1, 7.2, 7.3, 7.4 |
...
- Preconditions: current user loaded かつ non-empty access token。
- Postconditions: 成功・失敗・キャンセルのいずれでも current auth session を維持する。
...
| Requirements | 6.3, 6.4, 6.5, 6.7, 7.1, 7.2, 7.3, 7.4, 8.5, 8.6, 8.7, 8.8 |
...
- `PasskeyBeginResponse`: `challengeID` と `options` を保持する。`options` は registration / authentication の両方で server JSON subset を decode する。
- `PasskeyPublicKeyCredentialCreationOptions`: `rp.id`、`user.name`、`user.id`、`challenge`、`excludeCredentials` を保持する。`challenge` と `user.id` は base64url → `Data` へ変換して platform registration request に渡す。
- `PasskeyPublicKeyCredentialRequestOptions`: `rpId`、`challenge`、`allowCredentials` を保持する。discoverable login で allowCredentials が空の場合も許容する。
...
- `PasskeyRegistrationFinishResponse`: current server contract では `userID` のみ。`authCode` は optional として将来互換を持たせてもよいが、現行 flow は登録後に authentication を続ける。
...
ローカル永続データは追加しない。Platform passkey credential は iOS / iCloud Keychain 側が管理し、Feedman iOS は raw credential を永続化しない。Server side credential / challenge / AASA は #216 の責務である。
...
- Ceremony failure: `REGISTRATION_FAILED` / `AUTHENTICATION_FAILED`、AASA mismatch、expired challenge、unknown credential は credential 不整合を作らず retry 可能 error にする。
- Token exchange failure: passkey finish が成功しても token exchange に失敗した場合は既存 login failure と同様に authenticated transition しない。
- Account add failure: current session と refresh token を保持する。Add flow 失敗を logout / account deletion と混同しない。

### Error Categories and Responses

- **User Errors (4xx)**: empty username、invalid username、username taken、canceled passkey ceremony。入力修正または再試行を案内する。
- **System Errors (5xx)**: passkey endpoint 500、network failure、decode failure。既存文言に合わせ「時間をおいて再試行」を表示する。
...
- **Unit Tests**:
  - `PasskeyRepositoryTests`: 6 endpoint の method/path/body/Bearer header、401 refresh retry 委譲、DTO encode/decode。
  - `PasskeyPlatformAuthorizationCoordinatorTests`: base64url decode、registration/assertion credential envelope conversion、cancellation mapping。
  - `LoginViewModelTests`: passkey login success/failure/cancel、signup validation、registration → authentication → token exchange、duplicate guard、Google regression。
  - `AccountViewModelTests`: add begin/finish success、cancel/failure session preservation、missing token、duplicate guard、delete state 不変。
  - `AccountDisplayUser` tests: username optional fallback。
- **Integration Tests**:
...
- **E2E/UI Tests**:
  ...
  - Manual smoke: production/staging backend と AASA が有効な実機で signup、login、add passkey、timeline/feed/article detail、account deletion route を確認する。
- **Performance/Load**:

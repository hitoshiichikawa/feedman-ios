...
- [ ] 1. Passkey API model と repository 境界を追加する
  ...
  - `PasskeyRegistrationFinishResponse` は `authCode?` と `credentialID?` を扱い、登録直後 handoff で空の discoverable login に落ちない契約を表現する。
  - Add begin/finish は Bearer token を渡し、401 refresh retry は既存 `APIClient` hook に委譲する。
  - Repository tests で method/path/body/header、204 no-content、error propagation、distinct DTO encode/decode、`options.publicKey` missing decode failure、`UserResponse.username` fallback decode を検証する。
  - _Requirements: 2.3, 2.5, 2.6, 3.2, 3.4, 4.2, 4.4, 4.7, 6.6, 8.2, 8.3, 8.5, 8.6_
...
- [ ] 2. AuthenticationServices platform coordinator を追加する
  ...
  - Signup handoff の assertion は作成直後の `credentialID` を `allowCredentials` 相当に制限し、通常 login の discoverable flow と分離する。
  ...
  - Coordinator tests で base64url decode、required field missing、registration/assertion envelope conversion、cancellation mapping、AASA/entitlement mismatch、iOS 17.4+ exclude 反映、iOS 16〜17.3 fallback を mock で検証する。
...
- [ ] 3. LoginViewModel に passkey login / signup state machine を追加する
  ...
  - Passkey signup は username validation → registration begin → platform registration → registration finish → `auth_code` handoff または `credentialID` 付き passkey authentication continuation → token exchange の順に実行する。初回 registration request に recovery email は送らない。
  ...
  - ViewModel tests で signup empty username、`INVALID_USERNAME`、username taken、auth_code signup success、credential-bound signup success、registration cancel/failure/result-unknown、passkey login cancellation state、auth finish failure、token exchange failure、duplicate guard、Google regression、passkey flow が広告目的 event を出さないことを検証する。
...
- [ ] 5. Account sheet に passkey 追加登録 flow を追加する
  ...
  - Account tests で add success notice、cancel/failure/result-unknown session preservation、missing token、duplicate guard、logout/delete 共同 guard、401 refresh retry delegation、delete action visibility、deletion success/failure state 不変、`name` → `username` → `email` precedence を検証する。
...
- [ ] 7. Cross-boundary regression と最終検証を実行する
  ...
  - サーバ #216 が merge 済みであること、登録直後 handoff の `auth_code` または `credential_id` contract、production RP domain、AASA `webcredentials.apps` の確認結果または未確認事項を PR の「確認事項」に残す。
